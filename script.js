      ...
      return `
        <article class="product-card reveal" data-category="${catTypeAttr}" data-name="${cat.name}">
          <div class="product-card-img">
            <img src="${(cat.img && cat.img.startsWith('http')) ? cat.img : 'images/' + (cat.img || 'placeholder.png')}" alt="${cat.name}" loading="lazy" />
            <div class="product-card-overlay" onclick="window.openProductListing('${cat.name.replace(/'/g, "\\'")}')">
              <button class="product-card-overlay-btn" style="border:none; cursor:pointer;" aria-hidden="true" tabindex="-1">View Collection</button>
            </div>
          </div>
...
(function initFilter() {
  ...
  // Set transition on cards is now handled dynamically or by CSS directly.
})();
...
function renderCard(p, catData) {
  ...
  // If the img is a full Firebase Storage URL, use it directly; otherwise build a local path
  const imgSrc   = imgFile.startsWith('http') ? imgFile : `images/${catData ? catData.id : 'chandeliers'}/${imgFile}`;
    ...
    <div class="listing-card">
      ...
      <div class="listing-card-body">
        ...
        <div class="listing-card-cta">
          <button class="btn-primary" onclick="enquireProduct('${p.name.replace(/'/g, "\\'")}')" style="font-size:0.82rem;padding:0.65rem 1.4rem; border:none; cursor:pointer;">
...
window.openProductListing = openProductListing;
...
window.enquireProduct = function(productName) {
  ...
  setTimeout(() => {
    ...
    if (msgField) {
      msgField.value = "I would like to enquire about the " + productName + ".";
    }
  }, 150); // slight delay to allow body overflow reset
};
